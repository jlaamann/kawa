defmodule Kawa.Repo.Migrations.CreateSagas do
  ...
  def up do
    create table(:sagas, primary_key: false) do
      ...
      add :workflow_definition_id, references(:workflow_definitions, type: :uuid, on_delete: :restrict), null: false
    ...
    create constraint(:sagas, :sagas_status_check, 
      check: "status IN ('pending', 'running', 'paused', 'completed', 'failed', 'compensating', 'compensated')")
    create constraint(:sagas, :sagas_retry_count_check, 
    ...
    create index(:sagas, [:status], where: "status IN ('running', 'paused')", name: :idx_sagas_status)
